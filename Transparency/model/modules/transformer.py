...
class SelfAttentionWide(nn.Module):
    ...
    def forward(self, x):
        ...
        keys    = self.tokeys(x)   .view(b, t, h, e) # b - batch size, t -seq length, k -embedding dim, h -number of heads
...
class CTransformer(nn.Module):
    ...
    def __init__(self, emb, heads, depth, seq_length, num_tokens, max_pool=True, dropout=0.0, wide=False):
        """
        ...
        :param num_tokens: Number of tokens (usually words) in the vocabulary
         # :param num_classes: Number of classes. not needed, we will use BCEWithLogits
    ...
    def forward(self, x):
        ...
        x = self.tblocks(x)
        print("X SHAPE PRE POOL", x.shape)
